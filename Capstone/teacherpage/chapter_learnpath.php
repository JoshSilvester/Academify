...
<body>
    ...
    <div class="container">
        ...
        <div class="box">
            ...
            $path_query = "SELECT * FROM learning_path WHERE path_id = '$path_id'";
            $get_path = mysqli_query($con, $path_query);
            $path = mysqli_fetch_assoc($get_path);

            $get_query = "SELECT * FROM chapter WHERE id_chapter = '$id'";
            $data = mysqli_query($con, $get_query);
            $display = mysqli_fetch_assoc($data);
            // echo '<a href="path_material.php.php?id=' . $display['id_course'] . '" class="back-link">Back to Main Menu</a>';
            echo '<a href="path_material.php?id=' . $path['path_id'] . '" class="back-link">Back</a>';
            ...
            if (!empty($chapter_gdrive)) {
                echo ' <td><a href="' . $display['chapter_gdrive'] . '">Click this for ppt</a></td>';
            } else {
                echo ' <td><a href="">Not yet Available</a></td>';
            }
            echo '
